...
def load_jobs():
    ...
    scraper_script = SCRIPT_DIR / "job_scraper.py"
...
def call_llm_inference(prompt):
    ...
    try:
        llm_script = AGENT_DIR / "llm_inference.py"
        cmd = f"cat {temp_file} | {sys.executable} {llm_script} --chat"
        result = subprocess.run(
            cmd,
            shell=True,
            capture_output=True,
...
def call_ai_analyze_job(job_description):
    ...
    prompt_file = PROMPTS_DIR / "prompt_job_analysis.txt"
...
def build_filled_template_d(job_description):
    ...
    prompt_source = PROMPTS_DIR / "prompt_resume_generator.txt"

    # Step 1: Fresh copy of template
    print("📋 Copying template to filled_prompt.txt...")
    shutil.copy(prompt_source, "filled_prompt.txt")

    # Step 2: Verify copy matches original
    if filecmp.cmp(prompt_source, "filled_prompt.txt", shallow=False):
...
def format_cover_letter_with_script(analysis_json):
    ...
    script_path = UTILS_DIR / "json_to_cover_letter.py"
    result = subprocess.run(
        [sys.executable, str(script_path), "temp_analysis.json"],
        capture_output=True,
        text=True,
        cwd=os.getcwd()
    )
