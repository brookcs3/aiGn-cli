...
PROJECT_ROOT = Path("/Users/cameronbrooks/Server/AI-STEM-Separator-Mad-Scientist-Edition")
DATASET_DIR = PROJECT_ROOT / "dataset"
OUTPUTS_DIR = PROJECT_ROOT / "outputs"

# Default output for audit report (third-pass isolation)
DEFAULT_AUDIT_OUTPUT = PROJECT_ROOT / "outputs" / "third_pass" / "dataset_audit_report.md"
...
REPOMIX_PATHS = {
    ...
    "dataset": OUTPUTS_DIR / "repomix_dataset.md",
}

SPEC_PATHS = {
    "part1": OUTPUTS_DIR / "part1_the_separator_discriminative_prior.md",
    "part2": OUTPUTS_DIR / "part2_the_compressor_vae_encoder.md",
    "part3": OUTPUTS_DIR / "part3_the_guide_control_mechanism.md",
    "part4": OUTPUTS_DIR / "part4_the_generator_diffusion_transformer.md",
    "part5": OUTPUTS_DIR / "part5_the_synthesizer_vae_decoder.md",
}
...
class FileLoader:
    """Load and validate repomix and spec files."""
    ...
    def load_all(self) -> Dict[str, str]:
        ...
        logger.info("Loading repomix and spec files...")
        
        # Load repomix files (critical)
        for name, path in REPOMIX_PATHS.items():
        ...
        # Load spec files (critical)
        for name, path in SPEC_PATHS.items():
            content = self._load_file(path, critical=True)
            if content:
                self.loaded_files[f"spec_{name}"] = content
                self.file_sizes[f"spec_{name}"] = len(content)
        
        # Verify we have minimum required files
        if self.missing_files:
            missing_critical = [f for f in self.missing_files if "repomix" in f or "spec" in f]
            if missing_critical:
...
class ReportGenerator:
    ...
    def save_report(self, audit_result: str, pass1: str, pass2: str) -> Path:
        ...
        extraction_warnings, validation_failed = self._extract_and_save_files(audit_result)
        warning_section = self._format_warning_section(extraction_warnings, validation_failed)
...
"""
    ...
    def _strip_markdown_fences(self, content: str) -> Tuple[str, Optional[str], bool]:
        ...
        if "```" in stripped:
            truncated = stripped.split("```", 1)[0].rstrip()
