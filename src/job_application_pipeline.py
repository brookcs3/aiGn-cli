...
def load_jobs():
    ...
    if os.path.exists("src/output/jobs.csv"):
        os.remove("src/output/jobs.csv")
        print("🗑️  Deleted old src/output/jobs.csv")
    ...
    if result.returncode != 0:
        print("❌ src/job_scraper.py failed")
        sys.exit(1)
    if not os.path.exists("src/output/jobs.csv"):
        print("❌ src/job_scraper.py ran but jobs.csv still not found")
        sys.exit(1)
    print("\n✅ Fresh jobs loaded!\n")
    return pd.read_csv("src/output/jobs.csv")
...
def call_llm_inference(prompt):
    ...
    try:
        cmd = f"cat {temp_file} | python src/llm_inference.py --chat"
        print(f"   🔧 Command: {cmd[:80]}...")
        print("   ⏳ Waiting for LLM response (this may take 30-60s)...")
        result = subprocess.run(
            cmd,
            shell=True,
            capture_output=True,
        ...
        print(f"   ✅ LLM returned {len(result.stdout)} chars")
...
def call_ai_analyze_job(job_description):
    ...
    with open("src/prompts/prompt_job_analysis.txt", 'r') as f:
...
Please analyze and output JSON only."""
    ...
    with open("src/output/debug_ai_response.txt", 'w') as f:
        f.write(response_text)
    print(f"💾 Debug: saved to src/output/debug_ai_response.txt ({len(response_text)} chars)")
...
def build_filled_template_d(job_description):
    ...
    shutil.copy("src/prompts/prompt_resume_generator.txt", "src/output/filled_prompt.txt")

    # Step 2: Verify copy matches original
    if filecmp.cmp("src/prompts/prompt_resume_generator.txt", "src/output/filled_prompt.txt", shallow=False):
    ...
    with open("src/output/filled_prompt.txt", 'r') as f:
        content = f.read()

    # Step 4: Get questions data from debug_ai_response.txt
    with open("src/output/debug_ai_response.txt", 'r') as f:
    ...
    with open("src/output/filled_prompt.txt", 'w') as f:
        f.write(content)

    print("✅ src/output/filled_prompt.txt ready")
    return content
...
def call_ai_generate_resume(job_description):
    ...
    with open("src/output/debug_resume_response.txt", 'w') as f:
        f.write(response_text)
    print(f"💾 Debug: saved to src/output/debug_resume_response.txt ({len(response_text)} chars)")
...
def format_cover_letter_with_script(analysis_json):
    ...
    with open("src/output/temp_analysis.json", 'w') as f:
        f.write(analysis_json)

    # Call json_to_cover_letter.py
    result = subprocess.run(
        [sys.executable, "src/json_to_cover_letter.py", "src/output/temp_analysis.json"],
        capture_output=True,
        text=True,
        cwd=os.getcwd()
    )
    ...
    if result.returncode != 0:
        print(f"❌ src/json_to_cover_letter.py failed: {result.stderr}")
        sys.exit(1)

    cover_letter = result.stdout.strip()

    # Clean up temp file
    os.remove("src/output/temp_analysis.json")
