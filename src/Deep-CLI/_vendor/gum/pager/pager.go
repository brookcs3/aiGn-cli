...
type model struct {
	...
	maxWidth            int
	keymap              keymap
	fixedHeight         int // 0 = full screen, -1 = auto-fit content, >0 = fixed lines
}
...
func (m *model) processText(msg tea.WindowSizeMsg) {
	...
	// Account for viewport border height
	vpStyle := m.viewport.Style
	borderHeight := vpStyle.GetVerticalBorderSize() + vpStyle.GetVerticalMargins() + vpStyle.GetVerticalPadding()

	// Determine viewport height based on fixedHeight setting
	if m.fixedHeight == -1 {
		// Auto-fit: use content height, but cap at terminal height
		maxHeight := msg.Height - helpHeight - borderHeight
		if contentLines < maxHeight {
			m.viewport.Height = contentLines
		} else {
			m.viewport.Height = maxHeight
		}
	} else if m.fixedHeight > 0 {
		// Fixed height specified
		m.viewport.Height = m.fixedHeight
	} else {
		// Default: full screen
