...
# -- Magic Wrapper --
# Try to find magic in path or standard locations
if command -v magic &> /dev/null; then
    MAGIC_CMD="magic run python"
else
    # Fallback to python3 if magic not found (dev environment)
...
call_backend() {
    local script="$1"
    shift
    # Handle different script locations based on usage
    if [[ "$script" == "job_matcher.py" ]]; then
        $MAGIC_CMD "$JOBS_DIR/$script" "$@" 2>/dev/null
    elif [[ "$script" == "code_analyzer.py" ]]; then
        $MAGIC_CMD "$UTILS_DIR/$script" "$@" 2>/dev/null
    else
        $MAGIC_CMD "$CORE_DIR/$script" "$@" 2>/dev/null
    fi
}
...
while true; do
    ...
    case $OPTION in
        "Resume Analyzer")
            ...
            PROMPT_TEMP=$(mktemp)
            $MAGIC_CMD "$UTILS_DIR/resume_parser.py" --input-file "$RESUME_FILE" --output "$PROMPT_TEMP" 2>/dev/null

            # Run AI with spinner
            RESULT=$(cat "$PROMPT_TEMP" | gum spin --spinner pulse --title "Analyzing with AI..." -- $MAGIC_CMD "$CORE_DIR/llm_inference.py" --chat 2>/dev/null)
            rm -f "$PROMPT_TEMP"

            # Extract and flatten JSON from response with pandas
            JSON_RESULT=$(echo "$RESULT" | $MAGIC_CMD -c "
import sys, json, re
import pandas as pd
...
"
            ...
            GUM_CUSTOM="$TOOLS_DIR/gum/gum-custom"
...
$JOB_DISPLAY"
        ...
        "Generate Cover Letter")
            ...
            $MAGIC_CMD "$JOBS_DIR/job_application_pipeline.py" </dev/tty
        ...
        "Interview Prep Questions")
            ...
            # Generate with AI
            RESULT=$(cat "$PROMPT_TEMP" | gum spin --spinner pulse --title "Generating personalized questions..." -- $MAGIC_CMD "$CORE_DIR/llm_inference.py" --chat 2>/dev/null)
            rm -f "$PROMPT_TEMP"

            # Strip markdown code block wrappers if present
            RESULT=$(echo "$RESULT" | $MAGIC_CMD -c "
import sys, re
text = sys.stdin.read()
